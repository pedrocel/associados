...
@section('title', 'Planos - AssociaMe')
...
<div class="space-y-6">
        ...
        <div class="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
            <div>
                <div class="flex items-center space-x-3 mb-2">
                    ...
                    <h2 class="text-2xl font-bold text-gray-900 dark:text-white">Planos</h2>
                </div>
                <p class="text-gray-600 dark:text-gray-400">Gerencie os planos que seus clientes podem comprar.</p>
            </div>
        ...
        <div class="bg-white dark:bg-gray-800 rounded-xl p-6 border border-gray-200 dark:border-gray-700 hover:shadow-lg transition-shadow">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-2xl font-bold text-gray-900 dark:text-white">{{ $plans->total() }}</p>
                </div>
        ...
        <div class="bg-white dark:bg-gray-800 rounded-xl p-6 border border-gray-200 dark:border-gray-700 hover:shadow-lg transition-shadow">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-2xl font-bold text-green-600">{{ $plans->where('is_active', true)->count() }}</p>
                </div>
        ...
        <div class="bg-white dark:bg-gray-800 rounded-xl p-6 border border-gray-200 dark:border-gray-700 hover:shadow-lg transition-shadow">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-2xl font-bold text-red-600">{{ $plans->where('is_active', false)->count() }}</p>
                </div>
    ...
    <div class="space-y-4">
                ...
                <div class="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
                    <div class="flex items-center space-x-4">
                        ...
                        <div class="flex-1 min-w-0">
                            <h3 class="text-lg font-semibold text-gray-900 dark:text-white truncate">{{ $plan->name }}</h3>
                            <p class="text-sm text-gray-600 dark:text-gray-400">
                                <span class="font-medium">Preço:</span> R$ {{ number_format($plan->total_price, 2, ',', '.') }}
                            </p>
                            <div class="flex items-center space-x-4 mt-2">
                                ...
                                <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium
                                    ...
                                    <i data-lucide="{{ $plan->is_active ? 'check-circle' : 'x-circle' }}" class="w-3 h-3 mr-1"></i>
                    ...
                    <div class="flex items-center space-x-2">
                        <button onclick="copyLink('{{ $plan->hash_id }}')"
                                class="inline-flex items-center px-3 py-2 text-sm font-medium text-blue-600 hover:text-blue-700 dark:text-blue-400 dark:hover:text-blue-300 bg-blue-50 hover:bg-blue-100 dark:bg-blue-900/20 dark:hover:bg-blue-900/30 rounded-lg transition-colors">
        ...
        <div class="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-12 text-center">
            ...
            <h3 class="text-lg font-medium text-gray-900 dark:text-white mb-2">Nenhum plano encontrado</h3>
            <p class="text-gray-500 dark:text-gray-400 mb-6">Comece criando seu primeiro plano no sistema.</p>
            <a href="{{ route('associacao.plans.create') }}" 
    ...
    <div class="flex items-center justify-center min-h-screen p-4">
        <div class="bg-white dark:bg-gray-800 rounded-xl max-w-md w-full p-6 transform transition-all">
            ...
            <p class="text-gray-600 dark:text-gray-400 text-center mb-6">
                Tem certeza que deseja excluir o plano <span id="deletePlanName" class="font-medium"></span>? 
...
<script>
    ...
    function copyLink(hashId) {
        const link = `{{ route('checkout.show', ['hash_id' => '__hash_id__']) }}`.replace('__hash_id__', hashId);
        navigator.clipboard.writeText(link).then(() => {
            // Reutilizando a função de notificação que criamos antes
    ...
    function confirmDelete(planId, planName) {
        ...
        document.getElementById('deleteForm').action = `{{ route('associacao.plans.index') }}/${planId}`;
